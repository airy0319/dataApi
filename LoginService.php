...
class LoginService {
    public function PreLogin() {
        ...
        //认证信息
        $preLoginData = array('username' => USERNAME, 'token' => TOKEN, 'functionName' => 'preLogin', 'uuid' => UUID, 'request' => array('osVersion' => 'windows', 'deviceType' => 'pc', 'clientVersion' => '1.0',),);
    ...
    public function DoLogin() {
        ...
        $doLoginData = array('username' => USERNAME, 'token' => TOKEN, 'functionName' => 'doLogin', 'uuid' => UUID, 'request' => array('password' => PASSWORD,),);
        $doLogin->POST($doLoginData);

        if ($doLogin->returnCode === 0) {
            //这里php5.6.16一直报错, 修改成下面那句
//            $retData = gzdecode($doLogin->retData, strlen($doLogin->retData));
    ...
    public function DoLogout($ucid, $st) {
        ...
        $doLogoutData = array('username' => USERNAME, 'token' => TOKEN, 'functionName' => 'doLogout', 'uuid' => UUID, 'request' => array('ucid' => $ucid, 'st' => $st,),);
